...
const COMMANDS = ["help", "about", "projects", "whoami", "repo", "banner", "clear", "education", "experience", "achievement", "cp", "skills", "research", "connect"];
...
function commandHandler(input : string) {
  ...
  switch(input) {
    ...
    case 'banner':
      if(bareMode) {
        writeLines(["ShahidHasan.x69_x97", "<br>"])
        break;
      }
      writeLines(BANNER);
      break;
...
const initEventListeners = () => {
  ...
  // Delegated click handler for clickable suggestions in the banner
  document.addEventListener('click', (e) => {
    const target = e.target as HTMLElement;
    if (!target) return;

    const suggest = target.closest('.suggest') as HTMLElement | null;
    if (!suggest) return;

    const cmd = suggest.getAttribute('data-cmd');
    if (!cmd) return;

    // 'repo' / 'repos' should open GitHub
    if (cmd === 'repo' || cmd === 'repos') {
      writeLines(["Redirecting to github.com...", "<br>"]);
      setTimeout(() => {
        window.open(REPO_LINK, '_blank');
      }, 200);
    ...
    USERINPUT.value = cmd;
    // small timeout so the UI updates before executing
    setTimeout(() => {
      enterKey();
    }, 50);
  });
